...
class GuessView: UIViewController {
    ...
    @IBAction func ButtonClicked(_ sender: UIButton) {
        ...
        if(!isChecked) {
            if(self.countUserInput() < currPokemon!.name.count) {
                self.shakeEmptyInput()
            } else {
                if(userInput.joined(separator: "") == currPokemon!.name) {
                    print("Correct!")
                    self.view.bringSubviewToFront(correctPopup)
                    UIView.animate(withDuration: 0.2, animations: {
                        self.correctPopup.frame.origin.y = self.line.frame.origin.y
                    })
                    ...
                    print("Incorrect!")
                    self.view.bringSubviewToFront(incorrectPopup)
                    UIView.animate(withDuration: 0.2, animations: {
                        self.incorrectPopup.frame.origin.y = self.line.frame.origin.y
                    })
                }
                ...
                self.PokeImage.frame = CGRect(x: self.PokeImage.frame.origin.x - self.PokeImage.frame.size.width / 1.3, y: self.PokeImage.frame.origin.y - 10, width: self.PokeImage.frame.size.width * 1.7, height: self.PokeImage.frame.size.height * 1.7)
                self.PokeImage.center.x = self.view.center.x
            }
        } else {
            isChecked = false
            newPokemon()
    ...
    func setupScreen() {
        PokeImageYPos = self.PokeImage.frame.origin.y
        print("pokeimagestart", self.PokeImageYPos)
        self.PokeImage.frame = CGRect(x: self.PokeImage.frame.origin.x - self.PokeImage.frame.size.width / 1.3, y: self.PokeImage.frame.origin.y - self.PokeImage.frame.size.height / 1.3, width: self.PokeImage.frame.size.width * 1.7, height: self.PokeImage.frame.size.height * 1.7)
        ...
        let rightMargin = screenWidth * 0.9
        typeHolderLabel.frame.origin.x = leftMargin
        abilityHolderLabel.frame.origin.x = leftMargin
        typeLabel.frame.origin.x = leftMargin
        abilityLabel.frame.origin.x = leftMargin
        genHolderLabel.frame.origin.x = rightMargin - genHolderLabel.frame.size.width
        heightHolderLabel.frame.origin.x = rightMargin - heightHolderLabel.frame.size.width
        moveLabel.frame.origin.x = rightMargin - moveLabel.frame.size.width
        heightLabel.frame.origin.x = rightMargin - heightLabel.frame.size.width
        
        self.PokeImage.center.x = self.view.center.x
        self.PokeGuessrLabel.center.x = self.view.center.x
    }
    ...
    func setupGame() {
        ...
        for n in 0 ..< 16 {
            ...
            var yToSet = 0
            if(n == 0) {
                xToSet = Int(self.view.center.x) - 8 * Int(round(20 * self.buttonSizeMultiplier))
                yToSet = Int(yAxisScale-95)
            } else if(n < 8) {
                xToSet = Int(lettersToClick[n-1].frame.origin.x) + Int(round(40 * self.buttonSizeMultiplier))
                yToSet = Int(yAxisScale-95)
            } else if(n == 8) {
                xToSet = Int(self.view.center.x) - 8 * Int(round(20 * self.buttonSizeMultiplier))
    ...
    func newPokemon() {
        ...
        getPokemon {
            ...
            DispatchQueue.main.async {
                ...
                //hides the popups
                UIView.animate(withDuration: 0.2, animations: {
                    self.correctPopup.frame.origin.y = self.screenHeight
                    self.incorrectPopup.frame.origin.y = self.screenHeight
                })
                ...
                self.PokeImage.frame = CGRect(x: self.PokeImage.frame.origin.x + self.PokeImage.frame.size.width / 4, y: self.nameLabel.frame.origin.y + 40, width: self.PokeImage.frame.size.width / 1.7, height: self.PokeImage.frame.size.height / 1.7)
                self.PokeImage.center.x = self.view.center.x
                print("pokeimagetwo", self.PokeImageYPos)
    ...
    func shakeEmptyInput() {
        for n in 0 ..< userInput.count {
            if(userInput[n] == "/") {
                print("dam")
                //self.userInputButtonFrames.frame
                UIView.animate(withDuration: 0.065, animations: {
                    self.userInputButtonFrames[n].frame.origin.y -= 3
                })
                UIView.animate(withDuration: 0.13, delay: 0.065, animations: {
                    self.userInputButtonFrames[n].frame.origin.y += 6
                })
                UIView.animate(withDuration: 0.065, delay: 0.195, animations: {
                    self.userInputButtonFrames[n].frame.origin.y -= 3
                })
            }
        }
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        //hides the popups
        self.correctPopup.frame.origin.y = screenHeight
        self.incorrectPopup.frame.origin.y = screenHeight
        self.correctPopup.frame.size.width = self.view.frame.size.width
        self.incorrectPopup.frame.size.width = self.view.frame.size.width
        
        for n in self.correctPopup.subviews { n.center.x = correctPopup.center.x }
        for n in self.incorrectPopup.subviews { n.center.x = correctPopup.center.x }
        ...
        URLSession.shared.dataTask(with:url) { (data, response, err) in
            ...
            do {
                ...
                if(randomNum < 149) {
                    ...
                    self.currGen = 3
                } else if(randomNum < 488) {
                    self.currGen = 4
                } else if(randomNum < 639) {
                    self.currGen = 5
                } else if(randomNum < 707) {
                    self.currGen = 6
                } else {
                    self.currGen = 7
                }
